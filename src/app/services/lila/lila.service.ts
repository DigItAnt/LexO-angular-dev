...
export class LilaService {
  ...
  queryEtymon(label: string): Observable<any> {
    return this.http.get(this.baseUrl_document+'?sparqlQuery='+encodeURIComponent('PREFIX lime: <http://www.w3.org/ns/lemon/lime#> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX lemonEty: <http://lari-datasets.ilc.cnr.it/lemonEty#> SELECT  ?etymon ?language WHERE { ?etymon a lemonEty:Etymon ; rdfs:label '+label+' ; lime:language ?language . }&endpoint=')+this.endpoint)
  }
}
